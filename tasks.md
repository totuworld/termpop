...
## Phase 5: UI/UX 폴리싱
- [x] 말풍선 스타일 윈도우 (둥근 모서리, 꼬리 장식, 그림자)
- [x] 커서 위치 기반 팝업 — 호출한 터미널의 커서 바로 위에 표시
- [x] 글자 크기 조정 옵션 (--font-size, 설정 파일, 강의/프레젠테이션 용도)
- [x] 엔터키로 줄 바꿈이 가능하다는 점 하단에 안내 추가
- [x] Cmd + Shift 눌러야 입력된다는 점 안내 추가
- [x] 말풍선에서 Ctrl + +, Ctrl + -로 폰트 사이즈 즉시 변경 지원
- [x] 변경된 폰트 사이즈를 숫자로 즉시 피드백
- [x] 말풍선에서 Ctrl + 0 누르면 기본 폰트 사이즈로 복귀
- [x] 말풍선 안에서 폰트 사이즈 변경 관련해서 안내 추가
- [x] 말풍선 안에서 변경한 폰트 사이즈가 지속적으로 저장되어, 나중에도 같은 사이즈로 보여야함
